...
> 백엔드·인프라 관점의 핵심 의사결정과 트러블슈팅을 정리한 문서입니다. (2인 팀 / 손재이: 백엔드·인프라·배포 주담당)
...
| Flyway 마이그레이션 | 21개 |
| 교환 상태 머신 | 9개 상태 |
| 테스트 | 99개 (GitHub Actions CI + JaCoCo, 라인 커버리지 59%) |
...
- **스키마 진화 관리** — **Flyway 21개 마이그레이션**으로 컬럼 추가/제거·시드 데이터까지 버전 관리. 운영 DB의 재현 가능한 상태 보장.
...
- **결과** — 전체 테스트 green(현재 99개), 라인 커버리지 측정 체계 확보. "CI 부재 → 잠복 실패"를 구조적으로 차단.
...
- **테스트 커버리지** — 라인 커버리지 59% → 도메인 서비스 중심으로 70%대까지 보강 예정
